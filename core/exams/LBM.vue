...
<div class="canvases" style="position: relative; height: 240px; width: 600px;">
    <canvas ref="boltzmannCanvas" style="background-color: #9044FF; position: absolute; left: 0; top: 0;" width="600" height="240"></canvas>
    <canvas ref="vectorCanvas" style="position: absolute; left: 0; top: 0;; pointer-events: none" width="600" height="240"></canvas>
    <canvas ref="particleCanvas" style="position: absolute; left: 0; top: 0; pointer-events: none" width="600" height="240"></canvas>
    <canvas ref="barrierCanvas" style="position: absolute; left: 0; top: 0; pointer-events: none" width="600" height="240"></canvas>
</div>
<div id="controls" class="controls">   
    ...
    <label><input ref="flowvectors" type="checkbox" name="flowvectors"> 速度矢量</label>
    <label><input ref="flowparticles" type="checkbox" name="flowparticles"> 颗粒物</label>
    <button ref="playBtn">开始</button>
    <button ref="resetBtn">重置</button>
    <button ref="clearbarriers">清除障碍物</button>
    <br>
    <br>
    <label><input ref="viscosityCtl" type="range" name="viscosity" min="2" max="50"> 粘性系数</label><br><br>
    <label><input ref="speed" type="range" name="anim-speed" min="1" max="15"> 动画速度</label>
    <br><br>
    <label><input ref="flowspeed" type="range" name="flow-speed" value="0" min="0" max="100"> 流速调节</label>
</div>
<div style="border:1px solid gray; width: 600px; padding: 10px; margin-top:10px;">
    <p><b>左键</b> 拖动流体 <b>右键</b> 设障碍物</p>
</div>
</template>
...
const boltzmannCanvas=ref()
const vectorCanvas=ref()
const particleCanvas=ref()
const barrierCanvas=ref()

const drawmode=ref()
const viscosityCtl=ref()
const speed=ref()
const flowvectors=ref()
const flowparticles=ref()
const playBtn=ref()
const resetBtn=ref()
const clearbarriers=ref()
const flowspeed=ref()

const config={latticeWidth: 200,latticeHeight: 80,};
  
onMounted(()=>{
		var boltzcanvas = boltzmannCanvas.value;
    var vectorcanvas = vectorCanvas.value;
    var particlecanvas = particleCanvas.value;
    var barriercanvas = barrierCanvas.value;
